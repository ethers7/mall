...
- 注意：出于安全考虑，`document/sql/mall.sql`中不再包含示例账号（如`admin`、`member`）的密码散列，导入后示例账号处于“无密码”状态且无法登录，需自行设置密码，具体方式见[mysql环境搭建说明](document/reference/deploy-windows.md#mysql);
